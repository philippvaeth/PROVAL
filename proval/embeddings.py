...
d = 100
sw_alignment_train, sw_alignment_test = swTrainTest(train, test,
                                                    args.sw_alignment)

for embedding_type in [
        "sw_dissimrep", "protvec", "esm1b_tsvd", "cpcprot_cfinal_tsvd",
        "cpcprot_zmean_tsvd"
]:
    if not os.path.isfile("vecs/{}.p".format(embedding_type)):
        ...
        if embedding_type == "sw_dissimrep":
            embedded_X_Train, embedded_X_Test = swDissimRep(
                sw_alignment_train, sw_alignment_test, train, test, d)
        elif embedding_type == "sw_complex":
            embedded_X_Train, embedded_X_Test = swDissimRepComplex(
                sw_alignment_train, sw_alignment_test, train, test, d)
        elif embedding_type == "protvec":
            embedded_X_Train, y_train, embedded_X_Test, y_test = protvec(
                train, test)
        elif embedding_type == "esm1b_tsvd":
            embedded_X_Train, y_train, embedded_X_Test, y_test = esm1b(
                train, test)
        ...
        pickle.dump([embedded_X_Train, y_train, embedded_X_Test, y_test],
                    open("vecs/{}.p".format(embedding_type), "wb"))
