...
for filename in os.listdir("vecs"):
    if filename.endswith(".p") and filename != 'sw_complex.p': # tsne implementation does not support complex valued embeddings
        embedded_x_train, y_train, embedded_x_test, y_test = pickle.load(open("vecs/{}".format(filename), "rb"))
        y = np.concatenate(
            (np.array(list(y_test.values())), np.array(list(y_train.values()))), 0)
        vecs = np.concatenate(
            (np.array(
