...
def swDissimRep(X_Train, X_Test, train_fasta, test_fasta, d):
  ...
  embedded_X_Train_dict = {str(s.id):vec for s,vec in zip(train_fasta,embedded_X_Train)}
  embedded_X_Test_dict = {str(s.id):vec for s,vec in zip(test_fasta,embedded_X_Test)}
...
def esm1b(train, test):
    ...
    for s in train:
        if len(s.seq) <= 1022:
            ...
            with torch.no_grad():
                results = model(sample_batch_tokens.cuda(), repr_layers=[33], return_contacts=True)
